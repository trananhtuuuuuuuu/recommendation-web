"""Download cxycxyg/layoutlmv3-resume-en (section-level LayoutLMv3-base) and run
it on the 3 sample CVs, decoding its own 13-class flat labels. Uses the same
Tesseract OCR + box normalisation we feed our own model, for a fair comparison.
"""
...
BG = {"OTHERS"}          # background / non-entity class
MAXW = 90                # truncate span text when printing


def main() -> None:
    print("downloading cxycxyg/layoutlmv3-resume-en (weights only) ...", flush=True)
    d = snapshot_download(
        "cxycxyg/layoutlmv3-resume-en",
        allow_patterns=["layout-en/*"],
        ignore_patterns=["*optimizer*", "*scheduler*", "*rng_state*",
                         "*training_args*", "*trainer_state*"],
    )
    mdir = os.path.join(d, "layout-en")
    proc = AutoProcessor.from_pretrained(mdir, apply_ocr=False)
    ...
    for rel in CVS:
        ...
        for s in spans:
            ...
            print(f"[{s['order']:>4}] {s['label']:<22} {c:.2f}  {text!r}")
